import React, { Component } from 'react'
import { Table, Button, message } from 'antd'
import Api from '../services/Api'

const { Column } = Table
export default class ListaPessoas extends Component {

    constructor(props) {
        super(props)
        this.state = {
            pessoas: [],
            message: null
        }
    }

    componentDidMount() {
        this.refreshPessoas()
    }


    refreshPessoas() {
        Api.listarPessoas()
            .then(
                response => {
                    console.log(response)
                    this.setState({ pessoas: response.data })
                }
            )
    }

    success = (record) => {
        if (record.isVacinada) {
            record.isVacinada = false
        } else record.isVacinada = true
        Api.atualizarPessoa(record, record.codigo)
        message.success('Status alterado com sucesso!')
        let aux = this.state.pessoas
        const i = this.state.pessoas.findIndex(p => p.codigo === record.codigo)
        aux[i] = { ...aux[i], isVacinada: record.isVacinada }
        this.setState({ pessoas: aux })
    }

    render() {
        return (
            <div className="container">
                <h2>PESSOAS CADASTRADAS</h2>
                <div className="container">
                    <Table dataSource={this.state.pessoas}>
                        <Column title="NOME" dataIndex="nome" key="nome" />
                        <Column title="CPF" dataIndex="cpf" key="cpf" />
                        <Column title="TELEFONE" dataIndex="telefone" key="telefone" />
                        <Column title="EMAIL" dataIndex="email" key="email" />
                        <Column title="VACINAD@?" dataIndex="isVacinada" key="isVacinada"
                            render={(text, record) => (<p>{record.isVacinada ? "Sim" : "Não"}</p>)} />
                        <Column title="ATUALIZAR" key="atualizar"
                            render={(text, record) => (<Button onClick={() => this.success(record)}
                                type="primary">Alterar Status</Button>)}
                        />

                    </Table>
                </div>
            </div>
        )
    }

}